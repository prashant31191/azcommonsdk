...
public class TouchCircleView extends View {
    ...
    private static final int ANGLE_ANIMATOR_DURATION = 1000;//Rotating speed
    ...
    public static final int STATE_DRAW_PATH = 2;//Big round to small round gradient
    public static final int STATE_DRAW_OUT_PATH = 3;//Round to big round gradient
    ...
    public void setOutCirRadius(int outCirRadius) {
        ...
        Log.i("Default radius：", outCirRadius + "");
...
//        innerCirRadius = (int) (outCirRadius * 0.6f);
    ...
    private void drawCirclePath(Canvas canvas, float distance) {
        ...
        if (isDrawPath) {
            if (distance > 0) {
                p0.y -= changeDy;
                p11.y = p1.y = p0.y;//Larger vertical
                //Above +
                p5.y += distance;
                p7.y = p6.y = p5.y;
                //left +
